  ...
  module Admin
    class DashboardController < ::Admin::AdminController
      ...
      def journal_show
        start_date = parse_date(params[:start_date]) || 30.days.ago.to_date
        end_date = parse_date(params[:end_date]) || Date.current
        logs = JournalLog.where(used_on: start_date..end_date)

        render json: {
          summary: build_journal_summary(logs, start_date, end_date),
          daily_trend: build_journal_daily_trend(logs, start_date, end_date),
          trust_level_breakdown: build_journal_tl_breakdown(logs),
        }
      end
      ...
      def journal_users
        ...
        per_page = [[params[:per_page].to_i, 1].max, 100].min
        per_page = 20 if params[:per_page].blank?
        order = %w[total_requests last_active_on username trust_level].include?(params[:order]) ? params[:order] : "total_requests"
        direction = params[:direction] == "asc" ? "ASC" : "DESC"

        base_query = JournalLog.where(used_on: start_date..end_date)
        ...
        user_stats =
          base_query
            ...
            .select(
              ...
              "users.trust_level",
              "users.uploaded_avatar_id",
              "SUM(zotero_bridge_journal_logs.request_count) AS total_requests",
              "MAX(zotero_bridge_journal_logs.used_on) AS last_active_on",
            )
      ...
      def build_journal_summary(logs, start_date, end_date)
        today = Date.current
        seven_days_ago = 7.days.ago.to_date

        today_logs = JournalLog.where(used_on: today)
        week_start = [seven_days_ago, start_date].max
        week_logs = JournalLog.where(used_on: week_start..today)
      ...
      def build_journal_daily_trend(logs, start_date, end_date)
        daily_data =
          logs
            .group(:used_on)
            .select(
      ...
      def build_journal_tl_breakdown(logs)
        logs
          .joins(:user)
          .group("users.trust_level")
          .select("users.trust_level", "SUM(zotero_bridge_journal_logs.request_count) AS total_requests", "COUNT(DISTINCT zotero_bridge_journal_logs.user_id) AS user_count")
          .order("users.trust_level")
          .map do |row|
            {
              trust_level: row.trust_level,
